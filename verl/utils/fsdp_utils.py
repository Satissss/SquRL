...
def get_fsdp_wrap_policy(module, config=None, is_lora=False):
    ...
    Args:
        ...
        config: Configuration for wrap policy containing:
            - disable: Whether to disable wrap policy
            - transformer_layer_cls_to_wrap: Transformer layer classes to wrap
            - min_num_params: Minimum number of parameters for size-based wrapping
        is_lora: Whether to enable lambda policy for LoRA modules (PEFT fine-tuning)
    
    Returns:
        Auto wrap policy or None
    """
    if config is None:
        config = {}
    ...
    # This wraps leaf modules with trainable weights (LoRA adapters)
    if is_lora:
        def lambda_policy_fn(module):
            # Wrap if it's a leaf module (no children) with trainable weights
    ...
    # Add size-based policy if min_num_params is specified
    if min_num_params > 0:
        size_policy = functools.partial(size_based_auto_wrap_policy, min_num_params=min_num_params)
        policies.append(size_policy)
    
    # Add transformer-based policy if transformer classes are specified
    elif fsdp_transformer_layer_cls_to_wrap is not None:
        transformer_cls_to_wrap = set()
        for layer_class in fsdp_transformer_layer_cls_to_wrap:
            transformer_cls = get_module_class_from_name(module, layer_class)
            if transformer_cls is None:
                raise Exception(f"Could not find the transformer layer class '{layer_class}' to wrap in the model.")
            else:
    ...
    # Combine all policies with OR logic
