...
def compute_grpo_outcome_advantage(token_level_rewards: torch.Tensor,
                                   ...
                                   index: torch.Tensor,
                                   epsilon: float = 1e-6,
                                   group_scalar: torch.Tensor = None):
    """
    ...
    Args:
        ...
        eos_mask: `(torch.Tensor)`
            shape: (bs, response_length)
        group_scalar: `(torch.Tensor)`, optional
            shape: (bs,) - 每个样本对应的 group scalar，用于在标准化后乘以优势值
            计算公式: valid_num / (2N - Ndiff)，其中 valid_num 是该 instance 中 score > 2.5 的数量
    ...
    with torch.no_grad():
        ...
        # 在标准化之后，如果提供了 group_scalar，则乘以对应的标量
        if group_scalar is not None:
            scores = scores * group_scalar
...
def compute_policy_loss(old_log_prob, log_prob, advantages, eos_mask, cliprange, entropy_top_mask=None):
    ...
    """
    negative_approx_kl = log_prob - old_log_prob  # shape: (bs, response_length)
    ...
    clip_pg_losses = torch.maximum(pg_losses, pg_losses2)
    if entropy_top_mask is  None:
        pg_loss = verl_F.masked_mean(clip_pg_losses, eos_mask)
    else:
        pg_loss = verl_F.masked_mean(clip_pg_losses, eos_mask*entropy_top_mask)
