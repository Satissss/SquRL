...
class DataProto:
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, (slice, list, np.ndarray)) or (isinstance(item, torch.Tensor) and item.numel() > 1):
        # Ensure non_tensor_data maintains dtype=object for arrays
            for key in non_tensor_data:
                if isinstance(non_tensor_data[key], np.ndarray) and non_tensor_data[key].dtype != object:
                    non_tensor_data[key] = non_tensor_data[key].astype(object)
